...
DatasetLoader::DatasetLoader(const Config& io_config, const PredictFunction& predict_fun, int num_class, const char* filename,vector<string>& header)
  ...
  SetHeader(filename,header);
}

DatasetLoader::~DatasetLoader() {
}

void DatasetLoader::SetHeader(const char* filename,vector<string>& header) {
...
Dataset* DatasetLoader::LoadFromFile(const char* filename, int rank, int num_machines,vector<vector<double>>& data) {
  ...
  if (bin_filename.size() == 0) {
      ...
      if (!config_.two_round) {
          // read data to memory
          // auto text_data = LoadTextDataToMemory(filename, dataset->metadata_, rank, num_machines, &num_global_data, &used_data_indices,data,header);
          num_global_data = data.size();
          // dataset->num_data_ = static_cast<data_size_t>(text_data.size());
          dataset->num_data_ = static_cast<data_size_t>(data.size());
          // sample data
          auto sample_data = SampleTextDataFromMemory(data);
          CheckSampleSize(sample_data.size(),
                          static_cast<size_t>(dataset->num_data_));
          // construct feature bin mappers
          ConstructBinMappersFromTextData(rank, num_machines, sample_data, dataset.get());
          // initialize label
          dataset->metadata_.Init(dataset->num_data_, weight_idx_, group_idx_);
          // extract features
          ExtractFeaturesFromMemory(data, dataset.get());
          //  text_data.clear();
      } /*else {
...
std::vector<std::vector<double>> DatasetLoader::SampleTextDataFromMemory(const std::vector<std::vector<double>>& data) {
  ...
  std::vector<std::vector<double>> out(sample_indices.size());
...
void DatasetLoader::ExtractFeaturesFromMemory(std::vector<std::vector<double>>& ref_text_data, Dataset* dataset) {
  std::vector<std::pair<int, double>> oneline_features;
  double tmp_label = 0.0f;
  int col=ref_text_data[0].size();
  if (predict_fun_ == nullptr) {
    ...
    for (data_size_t i = 0; i < dataset->num_data_; ++i) {
     ...
     for(int idx=0;idx<col-1;idx++)
          oneline_features.push_back(std::make_pair(idx,ref_text_data[i][idx]));
      // set label
      dataset->metadata_.SetLabelAt(i, static_cast<label_t>(ref_text_data[i][col-1]));
      // free processed line:
...
std::vector<std::vector<double>> DatasetLoader::GetForcedBins(std::string forced_bins_path, int num_total_features,
                                                              const std::set<int>& categorical_features) {
