...
bool GBDT::ContinueTrain() {
  std::unique_ptr<Tree> new_tree(new Tree(2, false));
  Tree *newT = tree_learner_->Train(nullptr, nullptr);
  if(newT != nullptr)
    new_tree.reset(newT);
  else{
    return false;
  ...
  std::vector<double> init_scores(1, 0.0);
  if (new_tree->num_leaves() > 1) {
    should_continue = true;
    auto score_ptr = train_score_updater_->score();
    ...
    if (std::fabs(init_scores[0]) > kEpsilon) {
      new_tree->AddBias(init_scores[0]);
    }
  } else {
    // only add default score one-time
    if (models_.size() < static_cast<size_t>(num_tree_per_iteration_)) {
      double output = 0.0;
      if (!class_need_train_[0]) {
        if (objective_function_ != nullptr) {
          output = objective_function_->BoostFromScore(0);
        }
      } else {
        output = init_scores[0];
      }
      new_tree->AsConstantTree(output);
      // updates scores
      train_score_updater_->AddScore(output, 0);
      for (auto& score_updater : valid_score_updater_) {
        score_updater->AddScore(output, 0);
      }
    }
  }
  // add model
  ...
  if(iter_ >= config_->num_iterations){
    return true;
  }
  ++iter_;
  return false;
...
bool GBDT::TrainOneIter(const score_t* gradients, const score_t* hessians) {
  ...
  for (int cur_tree_id = 0; cur_tree_id < num_tree_per_iteration_; ++cur_tree_id) {
    ...
    if (class_need_train_[cur_tree_id] && train_data_->num_features() > 0) {
      ...
      Tree *newT = tree_learner_->Train(grad, hess);
      if(newT != nullptr)
        new_tree.reset(newT);
      else{
        return false;
