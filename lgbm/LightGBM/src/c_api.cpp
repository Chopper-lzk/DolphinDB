...
#include "./treelearner/parallel_tree_learner.h"
namespace LightGBM {
...
DDBApplication::DDBApplication(const char * parameters, Dataset* train_data) {
  ...
  if (config_.tree_learner != std::string("data")) {
    Log::Fatal("only support data parallel in c api");
  }
...
DDBApplication::~DDBApplication() {
 
}

bool DDBApplication::TrainOneIter() {
  return boosting_->TrainOneIter(nullptr,nullptr);
}
void DDBApplication::setSplitInfo(data_size_t num_data_in_leaf, double sum_gradients, double sum_hessians) {
  ((DBParallelTreeLearner*)(((GBDT*)(boosting_.get()))->getTreeLearner()))->setSplitInfo(num_data_in_leaf, sum_gradients, sum_hessians);
}
void DDBApplication::getSplitInfo(data_size_t* num_data_in_leaf, double* sum_gradients, double* sum_hessians) {
  ((DBParallelTreeLearner*)(((GBDT*)(boosting_.get()))->getTreeLearner()))->getSplitInfo(num_data_in_leaf, sum_gradients, sum_hessians);
}
bool DDBApplication::ContinueTrain() {
 return boosting_->ContinueTrain();
}
TrainStatus DDBApplication::getTrainStatus() {
  return ((DBParallelTreeLearner*)(((GBDT*)(boosting_.get()))->getTreeLearner()))->trainStatus;
}
void DDBApplication::setHisgotram(std::vector<FeatureHistogram*> FeatureHistograms){
  ((DBParallelTreeLearner*)(((GBDT*)(boosting_.get()))->getTreeLearner()))->smaller_leaf_histogram_array_reduce = FeatureHistograms;
}
std::vector<FeatureHistogram*> DDBApplication::getHisgotram(){
  return ((DBParallelTreeLearner*)(((GBDT*)(boosting_.get()))->getTreeLearner()))->smaller_leaf_histogram_array_reduce;
}

std::tuple<int, SplitInfo *, SplitInfo* > DDBApplication::getBestSplit(){
  std::tuple<int, SplitInfo *, SplitInfo* > bestSplit(config_.max_cat_threshold, &((DBParallelTreeLearner*)(((GBDT*)(boosting_.get()))->getTreeLearner()))->local_smaller_best_split, 
    &((DBParallelTreeLearner*)(((GBDT*)(boosting_.get()))->getTreeLearner()))->local_larger_best_split);
  return bestSplit;
  

}
void DDBApplication::setBestSplit(const char* smaller_buffer, const char* larger_buffer ){
  ((DBParallelTreeLearner*)(((GBDT*)(boosting_.get()))->getTreeLearner()))->local_smaller_best_split.CopyFrom(smaller_buffer);

  ((DBParallelTreeLearner*)(((GBDT*)(boosting_.get()))->getTreeLearner()))->local_larger_best_split.CopyFrom(larger_buffer);
  
} 
std::string DDBApplication::getModel(){
  return ((GBDT*)(boosting_.get()))->SaveModelToString(0, -1, config_.saved_feature_importance_type);
}
/*-----------------------------------------------------------------------------------------------------------------------------*/
...
class Booster {
 public:
  ...
  static void CheckDatasetResetConfig(
      ...
      const std::map<std::string, std::string>& new_param) {
    Config new_config;
...
int LGBM_DatasetCreateMain(const char* filename,
                               const char* parameters,
                               const DatasetHandle reference,
                               DatasetHandle* out,std::vector<std::vector<double>>& data,std::vector<std::string>& header) {
  API_BEGIN();
  auto param = Config::Str2Map(parameters);
  Config config;
  config.Set(param);
  ...
  DatasetLoader loader(config, nullptr, 1, filename,header);
  if (reference == nullptr) {
    if (Network::num_machines() == 1) {
      *out = loader.LoadFromFile(filename,data);
    } else {
      *out = loader.LoadFromFile(filename, Network::rank(), Network::num_machines(),data);
    }
  }
  /*else {
...
int LGBM_DatasetCreateFromSampledColumn(double** sample_data,
  ...
  DatasetLoader loader(config, nullptr, 1, nullptr,header);
...
int LGBM_BoosterPrintModel(BoosterHandle handle,
                                  int start_iteration,
                                  int num_iteration,
                                  int feature_importance_type, std::string& model) {
  API_BEGIN();
  Booster* ref_booster = reinterpret_cast<Booster*>(handle);
  model = ref_booster->SaveModelToString(
      start_iteration, num_iteration, feature_importance_type);
  API_END();
}
