...
struct SplitInfo;
class DDBApplication {
 public:
  DDBApplication(const char * parameters, Dataset* train_data);
  ...
  void setHisgotram(std::vector<FeatureHistogram*> FeatureHistograms);
  std::vector<FeatureHistogram*> getHisgotram();
  std::tuple<int, SplitInfo *, SplitInfo* > getBestSplit();
  void setBestSplit(const char* smaller_buffer, const char* larger_buffer );
  std::string getModel();
 private:
  /*! \brief All configs */
  Config config_;
 ...
 * \return 0 when succeed, -1 when failure happens
 */
LIGHTGBM_C_EXPORT int LGBM_DatasetCreateMain(const char* filename,
...
LIGHTGBM_C_EXPORT int  LGBM_BoosterPrintModel(BoosterHandle handle,
                                  int start_iteration,
                                  int num_iteration,
