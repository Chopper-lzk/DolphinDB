...
class DatasetLoader {
 public:
  LIGHTGBM_EXPORT DatasetLoader(const Config& io_config, const PredictFunction& predict_fun, int num_class, const char* filename,std::vector<std::string>& header);

  LIGHTGBM_EXPORT ~DatasetLoader();

  LIGHTGBM_EXPORT Dataset* LoadFromFile(const char* filename, int rank, int num_machines,std::vector<std::vector<double>>& data);

  LIGHTGBM_EXPORT Dataset* LoadFromFile(const char* filename,std::vector<std::vector<double>>& data) {
    return LoadFromFile(filename, 0, 1,data);
  }
 ...
 // Dataset* LoadFromBinFile(const char* data_filename, const char* bin_filename, int rank, int num_machines, int* num_global_data, std::vector<data_size_t>* used_data_indices);
  ...
  void SetHeader(const char* filename,std::vector<std::string>& header);
  ...
  std::vector<std::vector<double>> SampleTextDataFromMemory(const std::vector<std::vector<double>>& data);
 ...
 void ConstructBinMappersFromTextData(int rank, int num_machines, const std::vector<std::vector<double>>& sample_data, Dataset* dataset);

  std::vector<std::unique_ptr<BinMapper>> ConstructLocalBinMappersFromTextData(int rank, int num_machines, const std::vector<std::vector<double>>& sample_data, Dataset* dataset);
  /*! \brief Extract local features from memory */
  void ExtractFeaturesFromMemory(std::vector<std::vector<double>>& text_data, Dataset* dataset);
